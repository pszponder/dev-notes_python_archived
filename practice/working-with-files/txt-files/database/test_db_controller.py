...
class TestDBController(unittest.TestCase):
    def setUp(self):
        ...
        """
        self.db_name = "test_db.txt"
        self.test_todo_dict = {"id": 0, "msg": "test todo message", "complete": False}
        self.test_todo_str = f"{self.test_todo_dict['id']},{self.test_todo_dict['msg']},{self.test_todo_dict['complete']}\n"

        # Create a new txt file as the DB
        with open(self.db_name, "w") as file:
            file.write(self.test_todo_str)

        # Set the db_controller module's DB_NAME variable to self.db_name
